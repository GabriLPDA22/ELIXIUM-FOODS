  ...
  <article class="user-review-card bg-white rounded-lg border border-gray-200 p-6">
    ...
    <header class="user-review-card__header flex items-start justify-between mb-4">
      ...
      <div class="flex items-center gap-2">
        <button @click="$emit('edit', review)" class="text-gray-400 hover:text-gray-600 p-1 rounded transition-colors"
          title="Editar reseña">
        ...
        <button @click="$emit('delete', review)" class="text-gray-400 hover:text-red-600 p-1 rounded transition-colors"
    ...
    <div class="user-review-card__content">
      ...
      <div v-if="review.imageUrl" class="mb-4">
        <img :src="review.imageUrl" :alt="'Imagen de reseña'"
      ...
      <div class="flex items-center gap-4 text-sm text-gray-600">
        <span class="flex items-center gap-1">
          <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
              d="M14 10h4.764a2 2 0 011.789 2.894l-3.5 7A2 2 0 0115.263 21h-4.017a2 2 0 01-1.847-1.25L7 13m5-3V5a2 2 0 00-2-2H9.095c-.5 0-.905.405-.905.905 0 .714-.211 1.412-.608 2.006L9 7m5 3v10" />
          </svg>
    ...
    <div v-if="showImageModal" class="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4"
      ...
      <img :src="review.imageUrl" :alt="'Imagen de reseña'" class="max-w-full max-h-full object-contain" @click.stop />
      <button @click="closeImageModal" class="absolute top-4 right-4 text-white hover:text-gray-300 text-2xl">
        ✕
      </button>
    </div>
  </article>
